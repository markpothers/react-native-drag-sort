...
export default class AutoDragSortableView extends Component{
    ...
    constructor(props) {
        ...
        const dataSource = props.dataSource.map((item, index) => {
            ...
            const top = props.childrenHeights && props.childrenHeights.length == props.dataSource.length ? props.childrenHeights.reduce((acc, h, i) => i<index? acc+h : acc, 0) : parseInt((index/rowNum))*itemHeight;
    ...
    static getDerivedStateFromProps(nextprops, prevState) {
        ...
        if (nextprops.dataSource != prevState.curPropsDataSource || itemWidth !== prevState.itemWidth || itemHeight !== prevState.itemHeight) {
            ...
            const dataSource = nextprops.dataSource.map((item, index) => {
                const newData = {};
                const left = index % rowNum * itemWidth;
                const top = nextprops.childrenHeights && nextprops.childrenHeights.length === nextprops.dataSource.length ? nextprops.childrenHeights.reduce((acc, h, i) => i<index? acc+h : acc, 0) : parseInt((index/rowNum))*itemHeight;
    ...
    endTouch (nativeEvent) {
        ...
        if (this.touchCurItem) {
            ...
            Animated.timing(
                ...
                this.touchCurItem.ref.setNativeProps({
                    style: {
                        zIndex: this.props.reverseChildZIndexing ? this.state.dataSource.length-this.touchCurItem.index : defaultZIndex,
                    }
                })
        ...
        return this.state.dataSource.map((item,index)=>{
            ...
            return (
                <Animated.View
                    ...
                    style={[styles.item,{
                        zIndex: (this.props.reverseChildZIndexing ? this.state.dataSource.length-index : defaultZIndex),
...
AutoDragSortableView.propTypes = {
    ...
    childrenHeights: PropTypes.array,
    ...
    reverseChildZIndexing: PropTypes.bool,
